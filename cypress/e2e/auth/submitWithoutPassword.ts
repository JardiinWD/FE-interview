export const submitWithoutPassword = () => {
  it('Should Try to submit the Form without filled input (Password)', () => {
    ...
    const inputPasswordErrorMessage = cy.getElementByTestId(
      'user-password-error',
      'label',
      4000
    )
    inputPasswordErrorMessage.scrollIntoView().should('be.visible')
    ...
    submitErrorMessage
      .scrollIntoView()
      .should('be.visible')
      .should('contain.text', 'Request failed with status code 400')
  })
}
